...
def build_token_texts(model: Small_LLM_Model, vocab_size: int) -> list[str]:
    ...
    return cast(list[str], token_texts)
...
def generate_function_call_single_phase(
    ...
    selected_function = next(
        function
        for function in function_definitions
        if function.name == context.function_name_buffer
    )
...
def build_function_call_result(
    ...
    for name, value in parameters.items():
        parameter_type = function_definition.parameters[name].type
        if parameter_type == "number":
            parameters[name] = float(value)
        elif parameter_type == "integer":
            parameters[name] = int(value)
        elif parameter_type == "string":
            parameters[name] = preserve_literal_string_value(
                str(value),
                user_prompt
            )
...
def is_parameter_complete(context: DecoderContext) -> bool:
    """Check whether parameter generation has completed."""
    return context.state == DecoderState.PARAMETER_COMPLETE
...
def generate_parameters(
    ...
    token_counter = 0
    while (
        not is_parameter_complete(context)
        and token_counter < limit
    ):
        next_token_id, next_token_text = generate_next_token(
            model,
        ...
        for char in next_token_text:
            ...
            if context.state == DecoderState.PARAMETER_COMPLETE:
                break
    if not is_parameter_complete(context):
        raise ValueError(
